...
\committee{Pereira}{Adriano}{UFSM}
...
A interface \texttt{Teste.java} procura com seus métodos mapear as ações que serão necessárias para a execução de um teste \emph{Selenium} utilizando elementos da classe \emph{WebDriver}. 
...
\begin{enumerate}
	\item Alterar modelagem para permitir a realização de testes funcionais
	\item Inserção do Cucumber para descrever cenários para testes
	\item Implementar suporte para funções Ajax na solução
