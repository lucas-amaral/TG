...
Visando a produção de testes funcionais de aceitação, optou-se pela inclusão do \emph{framework} Cucumber na composição desta nova solução, somando-se assim as demais ferramentas já utilizadas até o atual momento. A mesma foi escolhida com base em sua interface amigável, que possibilita descrever um cenário completo e de fácil entendimento por todos os envolvidos do projeto, garantindo assim uma maior qualidade também quando pensamos em documentação das funcionalidades desenvolvidas em um sistema.
...
\chapter{Resultados}
Como forma de validação das abordagens criadas neste trabalho, onde as mesmas visam a redução do trabalho para um equipes de teste e desenvolvimento, realizou-se comparações que serviriam para levantar o ganho em utilizar uma abordagem ou outra. 

\section{Comparando as três abordagens propostas}
Em um primeiro momento, resolveu-se comparar as soluções propostas elencando os pontos fortes e fracos das mesmas, e com isso torna-se possível realizar escolhas sobre qual abordagem é mais apropriada para o contexto onde se encontra.
Por exemplo, quando a equipe decide que serão necessários apenas a realização de testes unitários, a abordagem que utiliza anotações apresenta-se como uma boa alternativa, pois a mesma apresenta recursos que permitem realizar este tipo de teste e ao mesmo tempo não 

\begin{table}[!htpb]
	\centering
	\begin{small}
	\begin{tabular}{|p{2cm}|p{7cm}|p{7cm}|}
		& Vantagens & Desvantagens \\ \hline
		Interface de anotação + Selenium &
		\begin{itemize} 
	 	...
	 	\begin{itemize}
	 		...
	 		\item Não suporta elementos que dependam de funções Ajax e Javascript
			\item Não retrocede o banco de dados ao ultimo estado válido após realizar os testes
			\item Poluição no código fonte pelo excesso de anotações nas classes
	 	\end{itemize}
	 	\\ \hline
		
		Cucumber + Selenium  & 
		\begin{itemize} 
			\item Suporta testes funcionais de aplicação 
			\item Cenários com linguagem próxima à linguagem natural 
			\item Necessário apenas descrever funcionalidades e cenários em arquivos .feature
			\item Suporte para testes com chamadas Ajax e Javascript
		\end{itemize} & 
		\begin{itemize}
			\item Não possibilita a realização de testes que dependam de condições
			\item Não permite a utilização de cenários dentro de outro cenário
			\item Não retrocede o banco de dados ao ultimo estado válido após realizar os testes
			\item Impossibilidade de utilizar parâmetros que diferem dos tipos básicos (int, float, String)
		\end{itemize} 
		\\ \hline
		...
		\begin{itemize} 
			\item Suporta testes funcionais de aplicação 
			\item Suporte para testes com chamadas Ajax e Javascript
			\item Possibilita a realização de testes condicionais
			\item Permite armazenar valores de um campo antes que o mesmo sofra alterações
		\end{itemize} & 
		\begin{itemize}
			\item Não retrocede o banco de dados ao ultimo estado válido após realizar os testes
			\item Um campo só pode conter um valor de teste
			\item Cenários com linguagem mais complexa que solução com Cucumber
			\item Não suporta elementos que dependam de funções Ajax e Javascript
		\end{itemize} \\
	\end{tabular}
...
A tabela a seguir apresenta algumas melhorias que poderiam ser incorporadas em uma nova versão deste projeto


\begin{table}[!htpb]
	\centering
	\begin{tabular}{|l|p{12cm}|c|}
		 & Descrição & Relevância \\ \hline
		 1 & Apresentar no auto completar do IDE, os campos pertencentes ao contexto atual do .feature  & Muito alta \\
		 2 & Não persistir no BD as alterações realizadas pelos testes automatizados & Alta \\
		 3 & Criar mecanismo que busque na árvore do projeto os Validators dos formulários e transcreva essas informações em cenários de teste cucumber & Alta \\
	\end{tabular}
	\caption{Melhorias sugeridas}
\end{table}
